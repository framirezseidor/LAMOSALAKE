CREATE OR REPLACE PROCEDURE CON.SP_CON_FCT_REV_BACKORDER_FOTO(
...
---------------------------------------------------------------------------------
 ...
 Descripción:        SP que transforma datos desde la capa PRE A CON para FCT_COM_REV_BACKORDER_FOTO
---------------------------------------------------------------------------------
*/
...
BEGIN
    ...
    BEGIN
        ...
        DELETE FROM CON.FCT_COM_REV_BACKORDER_FOTO
        WHERE
            SOCIEDAD_ID BETWEEN :SOCIEDAD_INICIO AND :SOCIEDAD_FIN  
            AND CANALDISTRIB_ID BETWEEN :CANAL_INICIO AND :CANAL_FIN
            AND FECHA_BACKORDER = LAST_DAY(DATEADD(MONTH, -1, CONVERT_TIMEZONE('UTC', 'America/Mexico_City', CURRENT_TIMESTAMP())))
            AND UNI_EST BETWEEN 
            COALESCE((
    ...
    BEGIN
        ...
        INSERT INTO CON.FCT_COM_REV_BACKORDER_FOTO (
            FECHA_BACKORDER,
            ANIO,
            MES,
            ANIOMES,
        ...
        SELECT
            LAST_DAY(DATEADD(MONTH, -1, CONVERT_TIMEZONE('UTC', 'America/Mexico_City', CURRENT_TIMESTAMP()))) AS FECHA_BACKORDER,
            EXTRACT(YEAR FROM LAST_DAY(DATEADD(MONTH, -1, CONVERT_TIMEZONE('UTC', 'America/Mexico_City', CURRENT_TIMESTAMP())))) AS ANIO,
            EXTRACT(MONTH FROM LAST_DAY(DATEADD(MONTH, -1, CONVERT_TIMEZONE('UTC', 'America/Mexico_City', CURRENT_TIMESTAMP())))) AS MES,
            EXTRACT(YEAR FROM LAST_DAY(DATEADD(MONTH, -1, CONVERT_TIMEZONE('UTC', 'America/Mexico_City', CURRENT_TIMESTAMP())))) * 100 +
            EXTRACT(MONTH FROM LAST_DAY(DATEADD(MONTH, -1, CONVERT_TIMEZONE('UTC', 'America/Mexico_City', CURRENT_TIMESTAMP())))) AS ANIOMES,
            SOCIEDAD_ID,
            ORGVENTAS_ID,
            CANALDISTRIB_ID,
            SECTOR_ID,
        ...
        SELECT COUNT(*) INTO :ROWS_INSERTED FROM CON.FCT_COM_REV_BACKORDER_FOTO
        WHERE ORGVENTAS_ID LIKE 'R%'
            AND SOCIEDAD_ID BETWEEN :SOCIEDAD_INICIO AND :SOCIEDAD_FIN  
            AND CANALDISTRIB_ID BETWEEN :CANAL_INICIO AND :CANAL_FIN
            AND UNI_EST BETWEEN 
    ...
    ---------------------------------------------------------------------------------
        ...
        VALUES('SP_CON_FCT_REV_BACKORDER_FOTO','FCT_COM_REV_BACKORDER_FOTO', :F_INICIO, :F_FIN,:T_EJECUCION ,:ROWS_INSERTED, :TEXTO );
    ...
    TRUNCATE TABLE CON.FCT_COM_REV_BACKORDER_ACT; --TRUNCAR ACTUAL PARA QUE NO HAYA DUPLICADOS
