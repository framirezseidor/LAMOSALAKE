...
BEGIN
    ...
    BEGIN
        ...
        --BORRAMOS SOLO LO QUE SE CARGO EN RAW ANIONES ORGVENTAS CANAL DISTRIB
        DELETE FROM PRE.PFCT_COM_ADH_VENTAS_PCP P
        USING ( SELECT DISTINCT ANIOMES, ORGVENTAS_ID, CANALDISTRIB_ID 
                FROM RAW.FILE_XLS_COM_ADH_VENTAS_PCP) R
        WHERE P.ANIOMES = R.ANIOMES AND P.ORGVENTAS_ID = R.ORGVENTAS_ID;
--        AND P.CANALDISTRIB_ID = R.CANALDISTRIB_ID;

        --BORRAMOS FECHA EN NULO
        DELETE FROM PRE.PFCT_COM_ADH_VENTAS_PCP 
        WHERE 
    ...
    BEGIN
        ...
        SELECT 
            ...
            'nvo' AS	MANDANTE	, 
            CONVERT_TIMEZONE( 'America/Mexico_City' , current_timestamp() )	AS	FECHA_CARGA	,
            RIGHT(CONVERT_TIMEZONE( 'America/Mexico_City' , current_timestamp() ),5)	AS	ZONA_HORARIA	
        FROM 
        ...
        WHERE
            ANIOMES IS NOT NULL AND ORGVENTAS_ID IS NOT NULL 
            AND CANALDISTRIB_ID IS NOT NULL 
