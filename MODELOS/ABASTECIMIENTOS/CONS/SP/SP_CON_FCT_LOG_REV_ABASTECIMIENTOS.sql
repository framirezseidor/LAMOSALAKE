CREATE OR REPLACE PROCEDURE CON.SP_CON_FCT_LOG_REV_ABASTECIMIENTOS()
...
---------------------------------------------------------------------------------
 ...
 Descripción:        SP FULL que transforma datos desde la capa PRE a CON para ABASTECIMIENTOS REV
---------------------------------------------------------------------------------
*/
...
BEGIN
    ...
    BEGIN
        ...
        CONCAT(MON_PED,'_',MON_LOC,'_',LAST_DAY(DATEADD(MONTH, -1, FECHA_CONTABILIZACION))) AS CLAVEMP_LOC, --CONCAT(MON_PED,'_',MON_LOC) AS CLAVEMP_LOC,
        CONCAT(MON_PED,'_USD','_',LAST_DAY(DATEADD(MONTH, -1, FECHA_CONTABILIZACION))) AS CLAVEMP_USD,      --CONCAT(MON_PED,'_USD') AS CLAVEMP_USD,
        CONCAT(MON_PED,'_EUR','_',LAST_DAY(DATEADD(MONTH, -1, FECHA_CONTABILIZACION))) AS CLAVEMP_EUR,      --CONCAT(MON_PED,'_EUR') AS CLAVEMP_EUR,
        ...
        AND CONCAT(SUBSTR(FECHA_DOCCOMPRAS, 0, 4), SUBSTR(FECHA_DOCCOMPRAS, 6, 2)) BETWEEN :FECHA_INICIO AND :FECHA_FIN
        ...
        CONCAT(MON_PED,'_',MON_LOC,'_',LAST_DAY(DATEADD(MONTH, -1, FECHA_CONTABILIZACION))) AS CLAVEMP_LOC,
        CONCAT(MON_PED,'_USD','_',LAST_DAY(DATEADD(MONTH, -1, FECHA_CONTABILIZACION))) AS CLAVEMP_USD,
        CONCAT(MON_PED,'_EUR','_',LAST_DAY(DATEADD(MONTH, -1, FECHA_CONTABILIZACION))) AS CLAVEMP_EUR,

        CASE 
            WHEN CLAVEOPERACION IN ('001','011','021', '004','014','024', '005','015','025', '041','051','061') THEN 'COLOCADAS'
            WHEN CLAVEOPERACION IN ('002','012','022','006','016','026') THEN 'CONTABILIZADAS'
            ELSE 'NA'
        END AS ESTADO,
...
;
    ...
    VALUES ('SP_CON_FCT_LOG_REV_ABASTECIMIENTOS','CON.FCT_LOG_REV_ABASTECIMIENTOS', :F_INICIO, :F_FIN, :T_EJECUCION, :ROWS_INSERTED, :TEXTO );
